...
#include "doe_expsquare.h"

void ESquare::init_design(vector<vector<double> > init)
{
    int i,j;
...
void ESquare::update_design(vector<vector<double> > init)
{
    int i,j;
    if(!init.empty())
    {
...
vector<vector<double> > ESquare::get_design()
{
    int i,j;
    for(j=0;j<nv;j++)
    {
...
void ESquare::evaluate_criteria()
{
    int i,j,k;
    obj = 0;
    CORR.assign(static_cast<unsigned long long int>(nv), vector<double>(nv, 0));
...
double ESquare::columnwise_exchange(int ncol, int ncp, vector<int> idx1,vector<int> idx2)
{
    int i1,i2,i,j;
